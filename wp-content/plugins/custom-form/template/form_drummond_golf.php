...
<div class="row">
	<?php 
		global $wpdb;
		$data_product = $wpdb->get_results("SELECT * from wp_product WHERE product_type = 1 AND deleted_at IS NULL");
		...
		<div class="product-box">
			<div class="detail-product">
				<div class="detail-img">
					<img src=" <?php echo $upload_url . $product->subdir .'/'. $product->image; ?>" title="Drummond Golf Product"> 
				</div>
				<div class="product-price">
					<h4 class="text-center">$<?= round($product->price,2) ?>
					<?php if(!empty($product->delivery_cost)):?>
					*
					<?php endif;?>		
					</h4>
				</div>
				<div class="product-detail text-center drummond-golf">
					<p><b class="product-name"><?= $product->name ?></b></p>
					<?php if(!empty($product->delivery_cost)):?>
				<div class="text-center shipping-fee-text"><p>*Items Incur a $<?= $product->delivery_cost ?> Shipping Fee</p></div>
				<?php endif;?>
				</div>		
